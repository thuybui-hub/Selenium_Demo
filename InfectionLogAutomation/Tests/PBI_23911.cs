...
namespace InfectionLogAutomation.Tests
{
    ...
    public class PBI_23911 : TestBase
    {
        ...
        public void PBI_23911_AT_23958()
        {
            ...
            Log.Info("4. Create new log entry for a resident that already had a log entry");
            LogEntryDetailPage.FillLogEntryInfo(logEntryData, "Resident");
            LogEntryDetailPage.btnSaveLogEntry.Click();

            Log.Info("Verify that duplicate record pop-up message opens with correct content");
            Assert.IsTrue(LogEntryDetailPage.CheckDuplicatePopup(logEntryData.Name + " " + logEntryData.MRN, "Resident"), "Duplicate popup content is incorrect");
            ...
            #region Edit page
            Log.Info("9. Go to New Log Entry -> Resident again");
            HomePage.SelectMenuItem(Constants.NewResidentLogEntryPath);
            ...
            Log.Info("4. Create new log entry for a resident that already had a log entry");
            LogEntryDetailPage.FillLogEntryInfo(logEntryData, "Client");
            LogEntryDetailPage.btnSaveLogEntry.Click();

            Log.Info("Verify that duplicate record pop-up message opens with correct content");
            Assert.IsTrue(LogEntryDetailPage.CheckDuplicatePopup(logEntryData.Name + " " + logEntryData.MRN, "Client"), "Duplicate popup content is incorrect");
            ...
            #region Edit page
            Log.Info("9. Go to New Log Entry -> Client again");
            HomePage.SelectMenuItem(Constants.NewClientLogEntryPath);
